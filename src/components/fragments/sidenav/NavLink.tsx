...
function NavLink({ href, title, icon, isParent, children }: NavItem) {
      ...
      {isParent ? (
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="master-data" className="border-none">
            ...
            >
              <span className={clsx({ "text-primary": isOpen })}>{icon}</span>
              <span className={clsx({ "text-primary": isOpen })}>{title}</span>
            </AccordionTrigger>
            <ScrollArea
              className={`hide-scroll-bar duration-300 ${isOpen ? "h-[360px]" : "h-0"}`}
            >
