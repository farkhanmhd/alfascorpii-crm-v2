...
export async function GET(request: NextRequest) {
  try {
    ...
    const searchFilter: Prisma.LeasingWhereInput = validatedSearch
      ? {
          OR: [{ nama: { contains: validatedSearch, mode: 'insensitive' } }],
        }
      : {};

    const [leasing, totalCount] = await prisma.$transaction([
    ...
    console.error('Error fetching leasing: ', error);
    return NextResponse.json({
      status: 500,
      message: 'Failed to fetch leasing data',
    });
  } finally {
